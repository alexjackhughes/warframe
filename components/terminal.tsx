...
export function Terminal({ messages, isTyping }: TerminalProps) {
  ...
  useEffect(() => {
    ...
    if (messages.length > 0 && messages[messages.length - 1].role === "assistant" && !isTypingResponse) {
    ...
    <div className="font-mono text-green-500 text-sm md:text-base relative">
          ...
          <div className="flex">
            <span
              className={cn(
                ...
                message.role === "assistant" && "text-green-500 font-bold",
            ...
            <span
              className={cn(
                "break-words",
                message.role === "system" && "text-yellow-500",
                message.role === "assistant" && "text-green-500",
                message.role === "user" && "text-green-300",
                index === currentMessageIndex && isTypingResponse && "blink",
              )}
            >
              {index === currentMessageIndex && isTypingResponse ? typedText : message.content}
            </span>
          </div>
        </div>
      ))}
      {isTyping && (
        <div className="flex">
          ...
          <span className={cn("text-green-500", cursor ? "opacity-100" : "opacity-0")}>_</span>
        </div>
      )}
    </div>
  )
