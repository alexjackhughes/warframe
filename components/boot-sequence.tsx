...
export function BootSequence() {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full w-full text-green-500 font-mono relative">
...
`}
        ...
        <div className="mb-6 text-center">
          <div className="text-xl font-bold mb-2">ADVANCED INTELLIGENCE SYSTEM</div>
          <div className="text-sm opacity-70">GOVERNMENT CLASSIFIED - TOP SECRET</div>
        </div>

        <div className="mb-6">
          {bootText.map((text, index) => (
            <div key={index} className={cn("mb-1", index === bootText.length - 1 && "blink")}>
              {text}
              {index === bootText.length - 1 && text !== "SYSTEM READY." && "..."}
            </div>
          ))}
        </div>
        ...
        <div className="text-xs opacity-70 mt-8">
          ...
          <div className="mt-2 text-yellow-500">WARNING: UNAUTHORIZED ACCESS IS A FEDERAL OFFENSE</div>
        </div>
      </div>
    </div>
  )
